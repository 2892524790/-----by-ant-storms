...
/*  r     h      N                  beta_01   beta_02    beta_03     b0     beta_0  beta_1     beta_2     N1     C    alpha1  alpha2  zeta    b*/
 {1000000 ,0.005 , 5,             300,      7000,      1000,      20,    10.8,   20.5,      0.0003,       200,    5,    0.95,   2,    0.03,    0.1},
 {1000000 ,0.005 , 5,             300,      7000,      1000,      20,    10.8,   20.5,      0.0003,       200,    5,    0.95,   2,    0.03,    0.1},
...
float Constrain_Float(float amt, float low, float high){
  return ((amt)<(low)?(low):((amt)>(high)?(high):(amt)));      //  限幅函数
}
...
{
    int16_t output=0;
    if(Input>1.0f) output=1;
    else if(Input<-1.0f) output=-1;
    else output=0;
    return output;
}     //符号
